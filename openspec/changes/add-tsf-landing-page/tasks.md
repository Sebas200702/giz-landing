...
- [x] 1.4 Crear `src/data/site.ts` con todo el copy (secciones, municipios, pasos, temarios, FAQs) y constantes de conversión (`ZOHO_FORM_URL`, contacto empresas) tipadas
- [x] 1.5 Extraer assets del bundle de referencia a `src/assets/images/`: logo TSF, foto de hero (recorte transparente oficial), logos de aliados (Cooperación Alemana/UE/GIZ), y favicon derivado del isotipo (mano+corazón) en `public/`; iconos de UI como componentes (`Icon.astro`)
...
- [x] 3.1 `Hero.astro`: fondo teal, kicker "Convocatoria abierta", `<h1>` "Cada trayecto merece una oportunidad", párrafo, CTAs (Quiero participar), foto LCP eager con `fetchpriority="high"`, layout responsive
...
- [x] 3.9 `Footer.astro`: banda teal de contacto + footer oscuro con lema y logos "Con el apoyo de" (incluye el 4º logo, Fundación Colombia Incluyente, recortado del PDF de assets)

## 4. Conversión (lead-capture)

- [x] 4.1
- [x] 4.2 `Registration.astro` (`#inscripcion`): iframe Zoho lazy con altura reservada y `title` cuando `ZOHO_FORM_URL` exista; estado "próximamente" 
- [x] 4.3
